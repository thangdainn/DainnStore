...
public class CurveLineChart extends JComponent {
    ...
    private final List<raven.chart.ModelLegend> legends = new ArrayList<>();
    private final List<raven.chart.ModelChart> model = new ArrayList<>();
    ...
    public void addLegend(String name, Color color1, Color color2) {
        raven.chart.ModelLegend data = new ModelLegend(name, color1, color2);
        legends.add(data);
        raven.chart.LegendItem legend = new raven.chart.LegendItem(data, legends.size() - 1);
    ...
    private void clearLegendSelected(raven.chart.LegendItem item) {
        item.setSelected(true);
        for (Component com : panelLegend.getComponents()) {
            if (com instanceof raven.chart.LegendItem) {
                raven.chart.LegendItem l = (LegendItem) com;
                if (l != item) {
                    l.setSelected(false);
                }
            }
